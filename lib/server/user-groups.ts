...
export type UserGroupRow = {
  ...
  /**
   * Rolün varsayılan izinlerinden çıkarılacak izinler. Boş dizi = hiçbir
   * şey çıkarılmaz (eski davranış). Bkz. resolvePermissionsForUser.
   */
...
/**
 * Bir kullanıcının nihai izin listesini hesaplar.
 *
 * Sıra önemli: önce rol varsayılanı + grubun EKLEDİĞİ izinler birleştirilir
 * (union, eski davranış), SONRA grubun ÇIKARDIĞI izinler bu birleşimden
 * düşülür. Bu sıra sayesinde "kasa erişimi olmayan resepsiyonist" gibi
 * roller mümkün olur: `reception` rolünün varsayılanında `cash.write` var,
 * ama grup `excludedPermissions: ['cash.write']` ile bunu kaldırabilir.
 *
 * `excludedPermissions` parametresi opsiyoneldir ve verilmezse (önceki
 * çağrı yerleri için) davranış tamamen eskisiyle aynıdır — hiçbir izin
 * çıkarılmaz.
 */
export function resolvePermissionsForUser(
  ...
  const base = groupPermissions.length === 0
    ? (ROLE_PERMISSIONS[role] ?? [])
    : [...new Set([...ROLE_PERMISSIONS[role], ...groupPermissions])];
  if (excludedPermissions.length === 0) return base;
...
function mapRow(r: {
  ...
  return {
    ...
    excludedPermissions: permissionsFromGroupJson(r.excludedPermissions),
    active: r.active,
  };
}
...
export async function saveUserGroupServer(
  ...
  const row = await prisma.userGroup.upsert({
    ...
    create: {
      ...
      excludedPermissions: JSON.stringify(data.excludedPermissions ?? []),
      active: data.active,
    },
    update: {
      name: data.name,
...
export async function getGroupPermissionsServer(
  groupCode: string,
  propertyId?: string,
): Promise<{ added: Permission[]; excluded: Permission[] }> {
  const groups = await getUserGroupsServer(propertyId);
  const group = groups.find((g) => g.code === groupCode);
  return { added: group?.permissions ?? [], excluded: group?.excludedPermissions ?? [] };
}
