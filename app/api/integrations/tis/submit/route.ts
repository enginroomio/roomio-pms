...
import { submitTisReport, testTisConnection } from '@/lib/integrations/tis/client';
...
export async function GET(req: Request) {
  ...
  const connection = await testTisConnection();
...
export async function POST(req: Request) {
  ...
  const result = await submitTisReport({ businessDate, reportId: body.reportId });
