...
export async function POST(req: Request) {
  ...
  if (body.action === 'delete') {
    ...
    try {
      ...
      logApiError('POST /api/users (delete)', err, { targetUserId: body.id });
  ...
  if (body.action === 'create') {
    ...
    try {
      ...
      // E-posta context'e PII olduğu için eklenmiyor; Sentry'ye sadece
      // hatanın oluştuğu işlem türü gönderiliyor.
      logApiError('POST /api/users (create)', err);
  ...
  if (body.action === 'reset-password') {
    ...
    try {
      ...
      logApiError('POST /api/users (reset-password)', err, { targetUserId: body.id });
  ...
  try {
    ...
    logApiError('POST /api/users', err);
