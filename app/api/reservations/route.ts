...
import { validateMarketForReservation, validateReservationNumericFields } from '@/lib/server/reservation-validation';
import { validateReservationDateFormats } from '@/lib/validation/reservations';
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const dateError = validateReservationDateFormats(body);
    if (dateError) return NextResponse.json({ error: dateError }, { status: 400 });
    const numericError = validateReservationNumericFields(body);
    if (numericError) return NextResponse.json({ error: numericError }, { status: 400 });
    ...
    logApiError('POST /api/reservations', err);
...
export async function PATCH(req: Request) {
  ...
  try {
    ...
    logApiError('PATCH /api/reservations', err);
