...
import { hasPropertyAccess } from '@/lib/auth/property-access';
...
export async function GET(req: Request) {
  ...
  if (payload) {
    ...
    if (!hasPropertyAccess(session, propertyId)) {
      return NextResponse.json(
        { ok: false, error: 'Bu şubeye erişim yetkiniz yok', authenticated: true },
        { status: 403 },
      );
    }
