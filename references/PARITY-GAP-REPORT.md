...
| Alan | Değişiklik |
|------|------------|
| **Çoklu şube yetki izolasyonu** | `requireApiPermission`/`requireApiAuth`/`requireApiAnyPermission` artık `x-roomio-property` header'ının kullanıcının erişim listesinde olup olmadığını doğruluyor (87 route otomatik kapsandı) — bkz. `lib/auth/property-access.ts` |
| **PostgreSQL şema senkronu** | `schema.postgresql.prisma` (21 model) ile `schema.prisma` (61 model) arasındaki 40 modellik fark kapatıldı. `scripts/sync-postgres-schema.mjs` ile otomatik üretiliyor; `npm run db:sync-pg-schema:check` artık `verify:all`'ın (→ `verify:ci`) ilk adımı — drift tekrar oluşursa pipeline kırmızı yanar |
