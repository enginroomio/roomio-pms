 ...
 *   sözdizimi/import taraması → şema senkron → typecheck → sunucu → deploy:checklist → E2E (auth, folio, entegrasyon, çoklu şube, i18n, roomio)
 *
  ...
  serverProc = spawn(
      ...
      : ['next', 'dev', '-p', PORT, '-H', '127.0.0.1'],
    {
      ...
      env: {
        ...
        // Testler aynı hesap/IP ile çok kısa sürede onlarca login isteği
        // gönderir (her test ayrı token alır) — gerçek saldırı değil, test
        // koşum deseni. Bu sunucu sadece verify akışında kullanılır.
...
async function main() {
  ...
  // node_modules henüz kurulmamışsa bile çalışan hafif statik kontroller
  // (TypeScript Compiler API ile sözdizimi + import doğrulama). `npm install`
  // sonrası `Typecheck` adımı çok daha kapsamlı olduğu için bunlar opsiyonel —
  // amaç, bağımlılıklar kurulamayan ortamlarda bile en azından sözdizimi/
  // yanlış import hatalarını erken yakalamaktır.
  run('Sözdizimi taraması (TS API)', 'npm', ['run', 'check:syntax'], { optional: true });
  run('Import doğrulama (TS API)', 'npm', ['run', 'check:imports'], { optional: true });
