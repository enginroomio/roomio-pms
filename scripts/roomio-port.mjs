...
export function killPort(port) {
  if (process.platform === 'win32') {
    // NOT: Önceki sürüm `for /f "tokens=5" %a in (...) do taskkill ...`
    // kullanıyordu. Bu sözdizimi BATCH DOSYASI içinde `%%a` gerektirir;
    // `execSync(cmd, { shell: true })` Windows'ta komutu `cmd /d /s /c`
    // üzerinden çalıştırır ve bu bağlamda `%a` davranışı tutarsızdır —
    // PowerShell veya bazı Node/Windows sürüm kombinasyonlarında sessizce
    // hiçbir süreci kapatmayabilir. `netstat` çıktısını burada (Node.js
    // tarafında) parse edip her PID için ayrı `taskkill` çağırmak —
    // scripts/roomio-kill-ports.mjs'de de kullanılan desen — sözdizimi
    // belirsizliğine bağlı kalmadığı için daha güvenilirdir.
    try {
      ...
      for (const pid of pids) {
        try {
          ...
          // ignore — kullanıcı Görev Yöneticisi'nden kapatabilir
        }
      }
    } catch {
      // port boş (findstr eşleşme yoksa non-zero exit verir)
    }
    return;
  }
