...
export function UserGroupsSettingsPanel() {
  ...
  const [editExcluded, setEditExcluded] = useState<Permission[]>([]);
    ...
    try {
      const res = await roomioFetch('/api/user-groups', {
        ...
        body: JSON.stringify({
          ...
          excludedPermissions: editExcluded,
        }),
      });
  ...
  function togglePerm(p: Permission) {
    ...
    // Bir izin eklenirken, aynı izin "çıkarılan" listesinde varsa kaldır —
    // bir iznin aynı anda hem eklenmiş hem çıkarılmış olması mantıksız
    // bir durum yaratırdı (çıkarma her zaman önceliklidir, ama UI'da
    // karışıklığı önlemek için burada da tutarlılığı koruyoruz).
    setEditExcluded((prev) => prev.filter((x) => x !== p));
  }

  function toggleExcluded(p: Permission) {
    setEditExcluded((prev) => (prev.includes(p) ? prev.filter((x) => x !== p) : [...prev, p]));
    setEditPerms((prev) => prev.filter((x) => x !== p));
  }

  function openPermissions(row: UserGroupRow) {
    setEditingCode(editingCode === row.code ? null : row.code);
    setEditPerms(row.permissions);
    setEditExcluded(row.excludedPermissions);
  }
    ...
    <div className="roomio-card">
          ...
          <tbody>
            {loading ? (
              ...
              <tr key={row.code}>
                ...
                <td>{row.name}</td>
                <td>{row.userCount}</td>
                <td title={`${row.permissions.join(', ')}${row.excludedPermissions.length ? ' | -' + row.excludedPermissions.join(', -') : ''}`}>
                  ...
                  {row.excludedPermissions.length
                    ? ` · -${row.excludedPermissions.length} ${t('kurulus.userGroups.excludedSuffix', undefined, 'çıkarılan')}`
                    : ''}
                </td>
      ...
      {editingCode ? (
        <div className="roomio-card" style={{ marginTop: 12, padding: 12 }}>
          ...
          <p className="roomio-page-desc" style={{ marginTop: 8 }}>
            {t('kurulus.userGroups.addedHint', undefined, 'Bu grubun rol varsayılanına EKLEDİĞİ izinler:')}
          </p>
          ...
          <p className="roomio-page-desc" style={{ marginTop: 16 }}>
            {t(
              'kurulus.userGroups.excludedHint',
              undefined,
              'Bu grubun rol varsayılanından ÇIKARDIĞI izinler (örn. kasa erişimi olmayan resepsiyonist):',
            )}
          </p>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginTop: 12 }}>
            {ALL_PERMISSIONS.map((p) => (
              <label key={`excl-${p}`} className="roomio-field roomio-field--checkbox" title={p}>
                <input
                  type="checkbox"
                  checked={editExcluded.includes(p)}
                  disabled={!canAdmin}
