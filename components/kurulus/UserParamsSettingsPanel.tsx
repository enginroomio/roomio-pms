...
import type { UserParamRow } from '@/lib/server/user-params';

export function UserParamsSettingsPanel() {
  const { t } = useI18n();
  const canAdmin = useKurulusAdmin();
  const [params, setParams] = useState<UserParamRow[]>([]);
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await roomioFetch('/api/user-params');
      const j = (await res.json()) as { params?: UserParamRow[] };
      setParams(j.params ?? []);
    } finally {
      setLoading(false);
    }
  ...
  async function save(key: string) {
    await roomioFetch('/api/user-params', {
      method: 'POST',
  ...
  return (
    <div className="roomio-card">
      <div className="roomio-kurulus-toolbar">
        <h2 className="roomio-card-title">{t('kurulus.userParams.title')} ({t('kurulus.live')})</h2>
      </div>
